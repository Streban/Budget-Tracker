...
export function SavingsTab() {
  ...
  const [isSavingsTrackerDialogOpen, setIsSavingsTrackerDialogOpen] =
    useState(false);
  ...
  const [editingSavingsTracker, setEditingSavingsTracker] =
    useState<SavingsTracker | null>(null);
  ...
  const totalGoals = savingsAccounts.reduce(
    (sum, account) => sum + account.goal,
    0
  );
  ...
  const totalAssetsValue = assets.reduce(
    (sum, asset) => sum + asset.amount * asset.currentRate,
    0
  );
  ...
  const handleSavingsTrackerSubmit = async (e: React.FormEvent) => {
    ...
    const selectedAccount = accounts.find(
      (account) => account.id === savingsTrackerFormData.accountId
    );
    if (!selectedAccount) {
    ...
    if (editingSavingsTracker) {
      ...
      await dataStore.updateSavingsTracker(
        editingSavingsTracker.id,
        trackerData
      );
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {totalGoals > 0
                ? ((totalSavings / totalGoals) * 100).toFixed(1)
                : 0}
              % of total goals
            </p>
          </CardContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Track your foreign currencies and other assets
              </CardDescription>
            </div>
            <Dialog
                ...
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingAsset
                      ...
                      : "Add a new currency or asset to track"}
                  </DialogDescription>
                </DialogHeader>
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    {editingSavings ? "Edit Savings Goal" : "Add Savings Goal"}
                  </DialogTitle>
                  <DialogDescription>
                    {editingSavings
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: string) =>
                        setSavingsFormData({ ...savingsFormData, type: value })
                      }
                    >
                      <SelectTrigger>
        ...
        <CardContent>
          <div className="space-y-6">
            {savingsAccounts.map((account) => {
              const progress =
                account.goal > 0 ? (account.balance / account.goal) * 100 : 0;
              return (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {account.type} • {account.bank} • {account.interestRate}
                        % APY
                      </p>
                    </div>
              ...
              {sortedAccounts.map((account) => (
                <TableRow key={account.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        account.type === "Credit" ? "destructive" : "outline"
                      }
                    >
                      {account.type}
                    </Badge>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Dialog
              open={isSavingsTrackerDialogOpen}
              onOpenChange={setIsSavingsTrackerDialogOpen}
            >
              <DialogTrigger asChild>
                <Button size="sm">
                  <Plus className="h-4 w-4 mr-2" />
                ...
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingSavingsTracker
                      ...
                      : "Track a deposit or withdrawal to your savings"}
                  </DialogDescription>
                </DialogHeader>
                <form
                  onSubmit={handleSavingsTrackerSubmit}
                  className="space-y-4"
                >
                  <div>
                  ...
                  <DialogFooter>
                    <Button
                      type="button"
                      variant="outline"
                      onClick={resetSavingsTrackerForm}
                    >
                      Cancel
                    </Button>
                    <Button type="submit">
                      {editingSavingsTracker ? "Update" : "Add"} Transaction
                    </Button>
                  </DialogFooter>
                </form>
              </DialogContent>
              ...
              {sortedSavingsTrackers.map((tracker) => (
                <TableRow key={tracker.id}>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {
                          accounts.find((acc) => acc.id === tracker.accountId)
                            ?.bank
                        }
                      </div>
                    </div>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        tracker.type === "deposit" ? "default" : "destructive"
                      }
                    >
                      {tracker.type}
                    </Badge>
                  ...
                  <TableCell
                    className={`text-right font-medium ${
                      tracker.type === "deposit"
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
                    {tracker.type === "deposit" ? "+" : "-"}
                    {formatPKR(tracker.amount)}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleDeleteSavingsTracker(tracker.id)}
                      >
                        <Trash2 className="h-3 w-3" />
                      </Button>
                    </div>
              ...
              {savingsTrackers.length === 0 && (
                <TableRow>
                  <TableCell
                    ...
                    className="text-center text-muted-foreground"
                  >
                    No savings transactions yet. Add your first transaction
                    above.
                  </TableCell>
                </TableRow>
              )}
